...
OD_MODEL_PATH     = r"C:\Users\KCCISTC\Documents\GitHub\safety_robot\YSY\best.pt"
...
def save_pr_curves(all_records, out_path):
    """미착용(positive=0) 기준 PR Curve — 헬멧 / 조끼 두 패널.
       MLC: sigmoid 점수로 threshold sweep -> 곡선 + AP(Average Precision).
       OD : 0/1 이산 출력 -> PR 평면 위 점 1개 (단일 운영 포인트).
       비교 메시지: 곡선이 OD 점의 위/오른쪽을 지나가면 MLC 가 적정 threshold 에서 OD 능가 가능."""
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    panels = [
        (axes[0], "gt_helmet", "mlc_helmet", "mlc_helmet_score", "od_helmet", "Helmet — 미착용 검출"),
        (axes[1], "gt_vest",   "mlc_vest",   "mlc_vest_score",   "od_vest",   "Vest — 미착용 검출"),
    ]
    for ax, gtk, mlc_bin_k, mlc_score_k, od_bin_k, title in panels:
        # GT None 인 person 은 평가 제외
        valid = [r for r in all_records if r[gtk] is not None]
        if not valid:
            ax.set_title(title + " (no data)")
            ax.axis("off")
            continue
        # "미착용 = positive(=1)" 로 라벨 뒤집기.  MLC 점수도 (1 - 착용확률) 로 뒤집어
        # "미착용일 확률" 로 변환.  OD 예측도 동일하게 (0/1) 뒤집어서 매칭.
        y_true_miss = np.array([1 - r[gtk] for r in valid])      # 1 = 미착용
        mlc_score_miss = np.array([1.0 - r[mlc_score_k] for r in valid])
        od_pred_miss = np.array([1 - r[od_bin_k] for r in valid])  # OD 의 미착용 예측 (0/1)
        ...
        ax.scatter([od_rec], [od_prec], color="#4C72B0", s=120, zorder=5,
                   marker="*", edgecolor="white", linewidth=1.2,
                   label=f"OD (P={od_prec:.2f}, R={od_rec:.2f})")

        # 기준선 — 양성 클래스 비율 (random classifier baseline)
        base = y_true_miss.mean()
        ax.axhline(base, color="gray", linestyle=":", linewidth=1,
                   label=f"baseline ({base:.2f})")

        ax.set_xlabel("Recall  (실제 미착용 중 잡아낸 비율)")
        ax.set_ylabel("Precision  (미착용 알람 중 실제 미착용 비율)")
...
def main():
    ...
    save_pr_curves(all_records, os.path.join(OUTPUT_DIR, "pr_curves.png"))
