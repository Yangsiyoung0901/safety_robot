...
│   ├── PPE_MobileNetV3Large.tflite  ← PPE 분류 모델 (MBC, 1명용)
...
IR 센서 감지 → 카메라 최신 프레임 → YOLO person 감지
  ...
  → 2명+: YOLO OD (best.pt, helmet/vest bbox)
  → 위험 표지판 감지 (best_p.pt) → 위험 구역 판정
  → 위반 시 스피커 경고 + 화면 표시
  → 브라우저 대시보드 실시간 업데이트
```
...
| 인원 | 방식 | 모델 |
|------|------|------|
| 1명 | MBC 분류 | PPE_MobileNetV3Large.tflite |
| 2명+ | YOLO OD | best.pt |
| MBC 실패 시 | OD fallback | best.pt |