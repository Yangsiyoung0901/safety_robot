...
JPEG_QUALITY = 95
MBC_THRESHOLD = 0.45   # MBC 분류 임계값 (올림: 현재 출력 확률 ~0.5로, 오탐 방지를 위해 상향)
MBC_OUTPUT_IS_LOGITS = True  # MobileNet 학습/변환 모델은 보통 logits 출력이므로 sigmoid 적용
...
def estimate_mbc_ppe_boxes(person_box, crop_box, frame_shape):
    """MBC 분류 결과에 표시용 helmet/vest 좌표를 붙이기 위한 추정 박스."""
    ...
    if crop_box is None:
        cx1, cy1, cx2, cy2 = px1, py1, px2, py1 + person_h * 0.82
    else:
        cx1, cy1, cx2, cy2 = [float(v) for v in crop_box]

    helmet_box = (
        max(0, px1 + person_w * 0.18),
        max(0, py1),
        min(w - 1, px2 - person_w * 0.18),
        min(h - 1, py1 + person_h * 0.24),
    )
    vest_box = (
        max(0, cx1 + (cx2 - cx1) * 0.12),
        max(0, py1 + person_h * 0.25),
        min(w - 1, cx2 - (cx2 - cx1) * 0.12),
        min(h - 1, min(cy2, py1 + person_h * 0.72)),
    )
    return helmet_box, vest_box
...
def detection_loop():
    ...
    # ── [수정] PPE 모델 클래스 검증 ──
    # helmet/vest 클래스가 하나도 없으면 PPE 모델이 아닌 것 (예: danger_sign 모델)
    if ppe_model and not ppe_class_ids:
        print(f"WARNING: PPE model at {ppe_model_path} has no helmet/vest classes!")
        print(f"  → classes found: {ppe_model.names}")
        print(f"  → PPE OD will be DISABLED. Please provide a correct PPE model (best.pt)")
        ppe_model = None
    ...
    if MBC_AVAILABLE and mbc_model_path:
        try:
            mbc_classifier = create_ppe_classifier(
                str(mbc_model_path),
                MBC_THRESHOLD,
                output_is_logits=MBC_OUTPUT_IS_LOGITS,
            )
    ...
    cached_mbc_results = {}  # {person_idx: {helmet, vest, h_prob, v_prob, helmet_box, vest_box, crop_box}}
    frame_index = 0
    fps = 0.0
    danger_hold = 0
    ...
    while not stop_event.is_set():
        ...
        if should_update_ppe:
            if use_mbc:
                ...
                helmet_box, vest_box = estimate_mbc_ppe_boxes(person["box"], crop_box, frame.shape)
                if crop_box is not None and not _is_head_only(crop_box):
                    ...
                    if crop.size > 0:
                        ...
                        if frame_index % 30 == 0:
                            print(
                                f"[MBC] helmet={helmet} (p={h_prob:.3f}), "
                                f"vest={vest} (p={v_prob:.3f}), crop={crop_box}"
                            )
                        cached_mbc_results[0] = {
                            "helmet": helmet,
                            "vest": vest,
                            "h_prob": h_prob,
                            "v_prob": v_prob,
                        ...
                        helmets = [{"box": helmet_box, "conf": h_prob, "center": center_of(helmet_box), "source": "mbc"}] if helmet else []
                        vests = [{"box": vest_box, "conf": v_prob, "center": center_of(vest_box), "source": "mbc"}] if vest else []
                        cached_ppe = {"helmets": helmets, "vests": vests}

                # MBC 크롭 실패 시 OD fallback
                if 0 not in cached_mbc_results and ppe_model is not None:
                    ...
                    for box in ppe_result.boxes:
                        ...
                        if cls_id in helmet_class_ids:
                            ...
                            vests.append(item)
                    cached_ppe = {"helmets": helmets, "vests": vests}
                elif 0 not in cached_mbc_results:
                    cached_ppe = {"helmets": [], "vests": []}
        ...
        for idx, person in enumerate(cached_persons, start=1):
            ...
            if use_mbc and (idx - 1) in cached_mbc_results:
                # MBC 결과 사용
                mbc = cached_mbc_results[idx - 1]
                has_helmet = mbc["helmet"]
                has_vest = mbc["vest"]
                helmet_box = mbc["helmet_box"] if has_helmet else None
                vest_box = mbc["vest_box"] if has_vest else None
                helmet_prob = mbc["h_prob"]
                vest_prob = mbc["v_prob"]
                person_method = "mbc"
                ...
                helmet_match = next((item for item in cached_ppe["helmets"] if point_in_box(item["center"], match_box)), None)
                vest_match = next((item for item in cached_ppe["vests"] if point_in_box(item["center"], match_box)), None)
                has_helmet = helmet_match is not None
                has_vest = vest_match is not None
                helmet_box = helmet_match["box"] if helmet_match else None
                vest_box = vest_match["box"] if vest_match else None
                helmet_prob = helmet_match["conf"] if helmet_match else None
                vest_prob = vest_match["conf"] if vest_match else None
        ...
        # OD는 실제 bbox, MBC는 사람 bbox 기반 추정 bbox를 표시한다.
        if use_od or use_mbc:
            for item in cached_ppe["helmets"]:
                x1, y1, x2, y2 = [int(v) for v in item["box"]]
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 1)
                draw_text(frame, "HELMET", (x1, max(18, y1 - 5)), (0, 255, 0), 0.45, 1)
            for item in cached_ppe["vests"]:
                x1, y1, x2, y2 = [int(v) for v in item["box"]]
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 180, 255), 1)
                draw_text(frame, "VEST", (x1, max(18, y1 - 5)), (0, 180, 255), 0.45, 1)
            ...
            data = {
                ...
                "persons": [
                    {"num": p["num"], "has_helmet": p["has_helmet"],
                     ...
                     "method": p["method"], "helmet_box": p["helmet_box"],
                     "vest_box": p["vest_box"], "helmet_prob": p["helmet_prob"],
                     "vest_prob": p["vest_prob"]}
