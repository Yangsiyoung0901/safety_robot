...
# - best_p.pt               : 표지판 탐지 모델 (류상균 팀원한테 받기)
...
#      danger_detector: "best_p.pt"  <- 표지판 모델
...
def is_in_danger_zone(person_bbox, sign_bboxes, expand=150):
...
# 7. 오버레이 그리기 함수
# =============================================================================
def draw_danger_zones(frame, sign_bboxes, expand=150):
    for x1, y1, x2, y2 in sign_bboxes:
        zx1 = max(0, x1 - expand)
        zy1 = max(0, y1 - expand)
        zx2 = min(frame.shape[1], x2 + expand)
        zy2 = min(frame.shape[0], y2 + expand)
        cv2.rectangle(frame, (zx1, zy1), (zx2, zy2), (0, 0, 255), 2)
        cv2.putText(frame, "DANGER ZONE",
                    (zx1, zy1 - 8), cv2.FONT_HERSHEY_SIMPLEX,
                    0.55, (0, 0, 255), 2)
    return frame
...
def draw_persons(frame, person_bboxes, sign_bboxes, expand=150):
    for bbox in person_bboxes:
        x1, y1, x2, y2 = bbox
        in_danger, _ = is_in_danger_zone(bbox, sign_bboxes, expand)
        if in_danger:
...
def main():
    ...
    try:
        ...
        while True:
            ...
            # -- 오버레이 그리기 --
            frame = draw_danger_zones(frame, sign_bboxes)
            frame = draw_persons(frame, person_bboxes, sign_bboxes)           
